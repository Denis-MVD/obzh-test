...
def save_result_to_file(data):
    ...
    # Используем utf-8-sig для корректного открытия в Excel на Windows
...
questions_10 = [
    ...
    ("Подготовка дома к наводнению?", ["Открыть окна", "Ценные вещи на чердак", "Ничего"], "Ценные вещи на чердак"),
...
questions_11 = questions_10 # Можно заменить на свои вопросы для 11 класса
...
if not st.session_state.test_started and not st.session_state.test_finished:
    ...
    if st.button("НАЧАТЬ ТЕСТ 🚀"):
        if name:
            ...
            # Перемешиваем вопросы, чтобы у соседа был другой порядок
    ...
    with st.expander("📊 ВХОД ДЛЯ УЧИТЕЛЯ"):
        ...
        if pin == TEACHER_PIN:
            if os.path.exists(RESULTS_FILE):
                ...
                st.write("### Подробная проверка")
                selected_user = st.selectbox("Выберите ученика:", df_res["ФИО"].unique())
                user_row = df_res[df_res["ФИО"] == selected_user].iloc[-1]
                
                details = user_row["Детализация"].split("|")
                for det in details:
                ...
                st.download_button("📥 СКАЧАТЬ ВСЮ ВЕДОМОСТЬ", data=csv_data, file_name="results_nvp.csv", mime="text/csv")
            else:
...
# --- ЭКРАН 3: РЕЗУЛЬТАТ ---
    ...
    st.header(f"Результат: {st.session_state.final_score} из 15")
